...
import { calculateDirectCommissionRate } from "./calculateDirectCommisionRate";
...
export const calculateUserCommission = (
  ...
  const userWithStar = calculateUsersStar(
    users,
    ordersWithinMonth,
    orders,
    month,
  ...
  console.log("userWithStar", userWithStar);
  ...
  for (let order of ordersWithinMonth) {
    ...
    usersObject[buyerInfo.parent] &&
      usersObject[buyerInfo.parent].commission.push({
        ...
        commissionPercent: calculateDirectCommissionRate(order.productType)
          .rate,
        type: "direct",
      });
